...
function RestaurantReservationsPage() {
    ...
    const handleCancelReservation = async (reservationId) => {
        if (!reservationId) {
            console.error("Invalid reservation ID for cancellation.");
            alert("Error: Unable to cancel reservation. Invalid reservation ID.");
            return;
        }

        try {
            const token = localStorage.getItem("jwtToken");
            await api.delete(`/reservation-service/reservations/${reservationId}`, {
                headers: {Authorization: `Bearer ${token}`},
            });

            // Update the list by removing the canceled reservation
